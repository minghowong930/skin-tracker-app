...
export default function SkinTab() {
  ...
  const handleSave = async () => {
    ...
    // 關鍵防護：只有當「確實有新照片」時才更新照片欄位，防止未拍照時將舊照片洗成 null
    if (photoUrl) {
      payload[photoCol] = photoUrl
    }

    // onConflict 確保以日期為唯一基準，有資料就更新，沒資料就新增
    const { error } = await supabase.from('daily_logs').upsert(payload, { onConflict: 'log_date' })

    if (error) { 
      alert('儲存失敗: ' + error.message) 
    } else { 
      alert(`${period} Skin 紀錄已儲存！`)
      setPhotoFile(null)
    ...
    <div className="pt-4 pb-24">
      ...
      <div className="bg-white rounded-apple shadow-apple p-6 mb-6">
        ...
        <div className="grid grid-cols-2 gap-4">
          <button onClick={() => setPeriod('AM')} className={`py-4 rounded-apple font-medium transition-all ${period === 'AM' ? 'bg-apple-blue text-white shadow-md' : 'bg-gray-100 text-apple-gray'}`}>☀️ AM</button>
          <button onClick={() => setPeriod('PM')} className={`py-4 rounded-apple font-medium transition-all ${period === 'PM' ? 'bg-apple-blue text-white shadow-md' : 'bg-gray-100 text-apple-gray'}`}>🌙 PM</button>
        </div>
      </div>
      ...
      <div className="bg-white rounded-apple shadow-apple p-6 mb-6">
        ...
        <div className="flex flex-col items-center gap-4">
          {photoPreview ? <img src={photoPreview} alt="Preview" className="w-32 h-32 object-cover rounded-lg border border-gray-200" /> : <div className="w-32 h-32 bg-gray-100 rounded-lg flex items-center justify-center text-apple-gray text-3xl">📷</div>}
      ...
      <button onClick={handleSave} disabled={isSaving || isProcessingPhoto} className="w-full bg-apple-blue text-white py-4 rounded-apple font-semibold text-lg hover:opacity-90 transition-opacity disabled:opacity-50">
        {isSaving ? 'Saving...' : (isProcessingPhoto ? 'Processing...' : `Save ${period} Skin Record`)}
      </button>
    </div>
  )
}